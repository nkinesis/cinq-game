 ...
 * @author Ullmann
 */
public class JFGameCustom7 extends JDialog {
    ...
    String answerTime = null; //tempo de resposta do jogador (em segundos)
    String sequenceLength = null; //tamanho da sequência gerada
    String sequenceLimit = null;//range de números na sequência (contando a partir de zero)
    String sequenceTime = null; //tempo por item da sequência (em milissegundos)
    ...
    private void setCustom() {
        ...
        sequenceLimit = prefs.get(PREF_NAME_C, defaultValue);
        answerTime = prefs.get(PREF_NAME_D, defaultValue);
        sequenceLength = prefs.get(PREF_NAME_E, defaultValue);
        sequenceTime = prefs.get(PREF_NAME_F, defaultValue);
    }

    public JFGameCustom7() {
        initComponents();

        try {
            setCustom();
            at = Integer.parseInt(answerTime);
            st = Integer.parseInt(sequenceTime);
            slt = Integer.parseInt(sequenceLength);
            slm = Integer.parseInt(sequenceLimit);
        } catch (Exception ex2){
            System.out.println(answerTime + " " + sequenceLength + " " + sequenceLimit + " " + sequenceTime);
            ex2.printStackTrace();
        }
        
        this.addWindowListener(new stopThreadClose());
...
private class stopThreadClose extends WindowAdapter {  
    ...
    private void jbCheckHActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbCheckHActionPerformed
        try {
            ...
            answer.add(Integer.parseInt(ans[6]));
            ...
            if (lista.get(0).equals(answer.get(0)) && lista.get(1).equals(answer.get(1))
                    ...
                    && lista.get(6).equals(answer.get(6))) {
                win = true;
            } else {
                win = false;
            }
            ...
            System.out.println(jtAnswerH.getText());
            ex.printStackTrace();
            clear();
